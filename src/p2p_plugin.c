...
ncclResult_t ncclIbGetPhysProperties(int dev, ncclNetProperties_t* props) {
  ...
  if (p2p_plugin == NCCL_P2P_IB && ncclParamMrcMultiRecvEnable()) {
...
ncclResult_t nccl_p2p_ib_init(int *nDevs, int *nmDevs, ncclIbDev *ncclIbDevs, char *ncclIbIfName, union ncclSocketAddress *ncclIbIfAddr, pthread_t *ncclIbAsyncThread, ncclDebugLogger_t logFunction)
{
  ...
  if (ncclNIbDevs == -1) {
      ...
      for (int d=0; d<nIbDevs && ncclNIbDevs<MAX_IB_DEVS; d++) {
        ...
        for (int port_num = 1; port_num <= devAttr.phys_port_cnt; port_num++) {
            ...
            for (int dev = devOffset; dev < devCount; ++dev) {
              ...
              TRACE(NCCL_NET,"NET/IB: [%d] %s:%s:%d/%s provider=%s speed=%d context=%p pciPath=%s", d, devices[d]->name, devices[d]->dev_name, ncclIbDevs[ncclNIbDevs].portNum,
                NCCL_IB_LLSTR(portAttr.link_layer), ibProviderName[ncclIbDevs[ncclNIbDevs].ibProvider], ncclIbDevs[ncclNIbDevs].speed, context, ncclIbDevs[ncclNIbDevs].pciPath);

              // Create MRC context
              ncclIbDevs[ncclNIbDevs].mrcContext = NULL;
    ...
    INFO(NCCL_INIT|NCCL_NET, "NET/IB : Using %s; OOB %s:%s", line, ncclIbIfName,
      ncclSocketToString(ncclIbIfAddr, addrline, 1));
    *nDevs = ncclNIbDevs;
